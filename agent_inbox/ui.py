...
.hosted-account { margin-top: 22px; }
.hosted-account > a { display: block; margin-top: 14px; }
.hosted-account > p { overflow-wrap: anywhere; }
#connections textarea { font: 14px/1.7 ui-monospace, SFMono-Regular, monospace; margin-bottom: 12px; }
#key-error { color: #a52222; }
...
function updateHeader(){
  ...
  $('#project-overview').hidden=!human()||state.tab!=='mail';$('#compose').hidden=(human()&&!hosted)||!state.project||state.tab==='reservations';
...
async function refresh({quiet=false,append=false}={}){
  if(!state.project){$('#content').replaceChildren(element('div',hosted?'Connect an agent to start a project.':'Register an inbox to get started.','empty'));return;}
...
function renderOverview(){
  ...
  const stats=element('div',undefined,'project-stats');for(const [value,label] of [[o.agents.length,hosted?'participants':'agents'],[o.thread_count,'threads'],[o.message_count,'messages'],[o.reservation_count,'reservations']]){const s=element('span');s.append(element('strong',String(value)),document.createTextNode(label));stats.append(s);}
  const panel=element('details',undefined,'activity-panel');panel.open=activityOpen;panel.append(element('summary',hosted?'People & agents':'Agent activity'));const grid=element('div',undefined,'agent-grid');
  for(const agent of o.agents){const card=element('article',undefined,'agent-card'),name=element('a',agent.address);name.href=`#${enc(agent.address)}`;card.append(name);const person=hosted&&state.member.members.some(email=>email.split('@')[0]===agent.address.split('@')[0]);const seen=element('p',person?'Human account':'Last seen ');if(!person)seen.append(time(agent.last_seen_at));card.append(seen);
...
function renderThread(t,target){
  ...
  if(!human()&&!hosted){const address=state.address;tools.append(button('Mark read',async()=>{try{await api(`/v1/inboxes/${enc(address)}/threads/${enc(t.thread_id)}/read`,{});if(address===state.address&&!human()){state.threadData.delete(t.thread_id);await refresh();notice('Marked read.');}}catch(e){fail(e);}}));}target.append(tools);
